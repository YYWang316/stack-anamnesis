...
**Status:** IN PROGRESS — data-only binding generalized; **USDT bound** (2nd stablecoin) + **ETH bound** (first cross-type, L1 native coin) (2026-06-15). Three subjects + two `subject_type`s (`stablecoin`, `l1`) flow end-to-end at the MACHINE level (smoke-validated). Next: the L1 data-depth fetcher (flagged), §II.B module completeness, and the rigorous narrative 2-type validation (deferred testing chunk).
...
**Done 2026-06-15 (cross-type — ETH bound, the first NON-stablecoin / L1 native coin, smoke-validated).**
- **ETH bound** (`"eth"` registry entry): subject_type **`l1`** (→ playbook class B "L1 / general-purpose chain", which is a REAL module — 命门指标 real-fee-revenue-net-of-incentives + retained active addresses, P/F valuation anchor, red flags — not a stub), decimals 18, **no issuer / no sec_cik / no eth_contract** (native coin). Routes to the L1 module, NOT the stablecoin path.
- **Generalized:** added the `SOURCES_BY_TYPE` row for `l1` + `chain` = `_L1_SOURCES` (CoinGecko + CoinMarketCap only, fetcher CLI type `"chain"`). The analysis side needed NO change — the USDT generalization already made `_envelope_pattern` skip on-chain (no `eth_contract` → None) and SEC (no `sec_cik` → None); the bundle/filler/charts are field-driven.
- **Gracefully SKIPPED (not forced, not faked):** Alchemy / Etherscan (native coin has no ERC-20 contract to read), SEC EDGAR (no issuer), and **DefiLlama** (its `/stablecoins` endpoint does not cover a native coin). ETH's price · market cap · total supply · volume · rank come from CoinGecko/CMC by name; total_supply renders in **tokens** (`120.68M ETH`), not a USD-peg — no stablecoin-shaped unit assumption (the CoinGecko extractor was already subject-agnostic).
- **Stablecoin-shaped assumptions found + how handled:**
  - *Fetcher set keyed only to `stablecoin`* → GENERALIZED (added the `l1`/`chain` row).
  - *Supply-momentum / KEY SIGNAL leg derives from the DefiLlama stablecoin circulating series* → for an L1 there is no such series, so the leg is simply **ABSENT** this run (field-driven: no supply-momentum sub-table; class B doesn't use the KEY SIGNAL verdict). **FLAGGED follow-on:** an L1 supply-momentum / fee-revenue / TVL leg needs a NEW chain-level fetcher + extractor (DefiLlama has a chain-TVL endpoint, but the analysis-side `defillama.latest` parses the stablecoin SERIES shape — a new path = substantial infra, not in this chunk).
  - *On-chain supply via ERC-20 `totalSupply`* → N/A for a native coin; SKIPPED cleanly (supply comes from the aggregators).
- **ETH smoke (`orchestrate ETH --fetch --html --bundle`) — gates green:** 2 sources fetched (coingecko, coinmarketcap); alchemy/etherscan/defillama/sec_edgar all SKIPPED; scaffold + bundle + html produced; **Spot metrics sub-table only** (supply-momentum + issuer-financials absent); ⑤.1 clean, ④.3 passes; provenance footer renders (charts = 0 SVG, both data-absent → placeholders). Bundle `subject_type: l1`, zero USDC/USDT/Circle/stablecoin DATA contamination. (The only "Stablecoin" word in the scaffold HTML is the v3 skeleton's own VERSION NOTE prose — template authoring text the writer drops in the narrative flow, not data.)
- **Tests:** `test_resolve_eth_first_cross_type_l1_no_contract_no_issuer`, `test_eth_cross_type_routing_data_only`, `test_eth_live_path_smoke_l1_spot_only`. Full suite 497 passed / 4 skipped.

**Remaining (the original scope continues).**
- **L1 data-depth follow-on (NEW, flagged above):** a chain-level supply-momentum / fee-revenue / TVL fetcher + extractor so class B's 命门指标 (real fee revenue, retention) is machine-fed rather than `[MANUAL]`. Substantial infra — needs scoping.
- **Module-completeness:** §II.B "Docks into" lists mechanism/metrics/comparison/valuation but NOT risk-rows / thesis-breakers (module A has both). A class-B run would leave those two anchors unconsumed (fail-loud) unless the writer fills them or §II.B is extended — review when an L1 narrative is written.
- **Rigorous cross-type 2-type validation** — the narrative-quality gate (writer actually stacks class B vs A, fills the L1 sub-questions, no stablecoin bleed) is the deferred "testing" chunk; this chunk validated the MACHINE path (data + gates) only.
- **L2 / other subject_types** (class C, etc.) + the token-schema work (TD-017) remain.
...
## TD-047 — ④ renderer redesign (report-writer chain step ④) — CLOSED-AS-BUILT 2026-06-05

**Status:** CLOSED-AS-BUILT 2026-06-05 (④.1 + ④.1b + ④.2 + ④.3 all landed — see Progress). Extracted from TD-041's "Still ahead" tail; the report-writer chain is ① facts bundle = TD-041, ② orchestrator wiring + ③ writer brief = TD-042, render-html (first cut) = TD-040.
...
**Progress.** ④.1 done (2026-06-05): `analysis_layer/render/html.py` adopts the approved institutional-clean design (`meta/design/report_design_v1.html` — `:root` token system + `@media print` embedded verbatim, stays self-contained/stdlib-only/deterministic) and adds the coaching-strip backstop (`> GUIDANCE` / `> TRAP` / `> ↳ Cap check` blockquotes removed whole) + the ⭐→★ KEY SIGNAL display normalisation. **④.1b done (2026-06-05):** the inferable signature touches the design uses — verdict-row highlight (a table row with a ☑ cell → `tr.is-selected`, ☐ → `tr.is-dim`; scoped inside the table renderer so only the §5.5 KEY SIGNAL matrix is hit, never the ☑/☐ checkbox lists), coloured `+/−` deltas (a clearly-signed numeric/percent/$ token → `.pos`/`.neg`; conservative lookbehind leaves date hyphens, hyphenated words and en-dash ranges alone), and `.callout` conclusion blocks (a paragraph opening `One-line conclusion:` / `VERDICT:`, bold-wrapper tolerated). Remaining: ④.2 (chart placeholders + inline-SVG charts — CSS already dormant in the embedded stylesheet) and ④.3 (the locked-design/SHA validator backstop). **Reconcile note (TD-023):** the still-deferred bespoke hand-markup is `.part`/`.part-eyebrow` section grouping and `.subtitle` (cosmetic, lower value); ④.1 applies the design at the element level (`report-head` header, `.page`, `table.memo`/`.table-wrap`, `p.prose`, classed headings, `.legend`) and ④.1b adds the verdict/delta/callout semantics from content cues; a small clearly-marked supplemental CSS block harmonises the generic `<ul>`/`<blockquote>`/`<hr>`/h5-6 tags the renderer emits.

**④.2 done (2026-06-05):** subject-agnostic inline-SVG charts driven by the facts bundle. New pure module `analysis_layer/render/charts.py` with two field-driven builders — `supply_momentum_svg(facts["supply_momentum"])` (one signed bar per window, height ∝ `|net_change_pct|` around a zero baseline, positive→`var(--ok-ink)`/negative→`var(--bad-ink)`, labelled window + signed value) and `issuer_financials_svg(facts["issuer_financials"])` (flow `revenues`/`net_income` and stock `assets`/`liabilities`/`equity` in SEPARATE panels, each on its own scale so the two unit kinds never share a misleading axis — the ④.2 CAUTION; negative `net_income`→red, units + fiscal year labelled). Each reads ONLY bundle FIELD NAMES (never a USDC literal or subject) and returns an inline `<svg>` string or `None` when the data is absent. `render_html` gains `facts=` (the in-memory `build_facts_bundle` dict — read from memory, never disk; `facts=None` → no chart section, backward-compatible); `orchestrate._run` builds the bundle ONCE when `--html`/`--bundle` and passes it as `facts=` (the `.facts.json` file write still governed by `--bundle`, unchanged). **Placement (TD-023):** the design file parks `.chart-grid` INSIDE the On-Chain Metrics section, but that section has no stable anchor in the constrained markdown, so charts render in an **"At a glance"** panel right after the legend, before the body (the prompt's stated default — one canonical spot). **Graceful-missing:** a chart whose field is absent renders the design's dashed `.chart-ph` with a "no data this run" body + an honest data-derived caption, never a crash or fabricated chart; a filled slot gets an `is-filled` modifier (small supplemental CSS) that drops the placeholder hatch for a clean panel. Self-contained (inline SVG, no `xmlns`/`http`/`<script>`/`<link>`, no JS), stdlib-only, deterministic (fixed `viewBox` + fixed number formatting). Tested incl. the ★ GENERIC non-USDC "TESTCOIN" bundle (charts render from ITS windows/values), graceful-missing, and `facts=None`.

**④.3 done (2026-06-05) — ④ renderer redesign COMPLETE.** New pure module `analysis_layer/render/validate.py`: `validate_report_html(html, *, facts_present=False) -> list[str]` (PURE, stdlib-only, no-network; empty = clean) — the HTML-INTEGRITY gate, distinct from ⑤ (numbers/confidence, TD-048). Checks (all FAIL-CLOSED): (1) **no coaching leak** — a `GUIDANCE`/`TRAP`/`↳ Cap check` marker LEADING a `<blockquote>`; (2) **self-contained** — no `http(s)://` / `<script` / `<link` / `src=` / `@import` / `cdn`; (3) **design structure** — `class="page"`, `report-head`, `legend`, `<table class="memo">`, the `:root` token block, `@media print`; (4) **badges applied** — ≥1 `badge-(filled|warn|manual|tag)` span (soft); (5) **chart integrity** (only when `facts_present`) — the "At a glance" section exists + SVG geometry sane (no NaN/Inf/negative in numeric attrs); (6) **well-formed-ish** — `<!DOCTYPE html>` prologue + `<html>`/`</html>`. Wired FAIL-CLOSED in `orchestrate._run`: after `render_html`, `validate_report_html(...)` runs and a non-empty result RAISES **before** any file write (a leaking/external-pulling report is never shipped). Real USDC report passes → `[]` (calibrated), live path unaffected. **★ DEVIATION (TD-023):** the brief specified the coaching check as a bare substring test AND that the real report must return `[]` — those conflict, because the live v2 template front matter LEGITIMATELY names the channel in PRESERVED prose ("M5 GUIDANCE/TRAP channel…", "↳ Cap check pointers", inline `<code>&gt; GUIDANCE</code>`). The substance the gate protects is "no coaching BLOCKQUOTE leaked", and coaching is by definition a blockquote LED by the marker (what the ④.1 strip targets), so the check is anchored to a marker leading a blockquote — stays fail-closed against the real leak vector while not flagging descriptive prose (strictly better than downgrading the whole check to a warning). **★ Geometry false-positive guard:** the NaN/Inf/negative check extracts numeric tokens from SVG geometry ATTRIBUTE VALUES (`viewBox/width/height/x/y/x1…/points/d`, name not preceded by `\w`/`-` so `stroke-width` ≠ `width`) inside `<svg>…</svg>` regions only — never substring-matched against free text, so prose like "fi-**nan**-cials"/"cove-**nan**-t" is safe (the exact trap that bit ④.2). **Drift-guard (the "SHA" half):** `tests/.../test_validate_html.py::test_design_css_sha_pin` pins `sha256(_DESIGN_CSS)` = `48f4ef4b…e4bb8`; a design-CSS change fails the test with "design CSS changed — if intentional, update DESIGN_CSS_SHA", making drift a conscious, reviewed act. Tests: clean→[], each violation caught (leaked coaching blockquote, `<script>`/`https://cdn`, negative width / NaN coord, missing `report-head`), the geometry false-positive guard, `facts_present=False` skips chart-integrity, plus the SHA pin. **④ renderer redesign (TD-047) closed-as-built.**
...
**⑤.1 done (2026-06-06) — fail-closed numbers-trace (no-override of machine-filled values).** New pure package `analysis_layer/qc/` (distinct from `render/validate.py` ④.3): `qc/numbers.py::check_numbers_trace(report_md, facts) -> list[str]` (PURE, stdlib-only, no-network; `[]` = clean). Iterates the bundle's machine-authoritative fields SUBJECT-AGNOSTICALLY (spot `metrics`, `supply_momentum` %/abs, each non-null `issuer_financials` cell) and asserts each value is PRESENT in the final report in a canonical rendering; absent ⇒ violation. Null cells + MANUAL/narrative numbers are skipped (a floor, not a ceiling — a *contradicting* invented number is deferred to TD-049). Wired FAIL-CLOSED as **step 4 of the `/research` command** (a non-zero `python -m analysis_layer.qc.numbers <report.md> <facts.json>` STOPS before render); the real latest USDC `.report.md` passes → `[]`. **★ DEVIATION (TD-023 — verify reality before coding):** (a) **Wiring hook.** The brief pointed at a "final-report hook"; reality is the final, writer-authored narrative exists ONLY in the `/research` command, NOT in `orchestrate.py` (which emits the deterministic *scaffold* — tracing there is trivially true). Wired in the command and flagged inline. (b) **Formatting parity.** The brief said "reuse the filler's formatting (`-1.62%`, `$2.75B`, `$76.5B`)", but `fill.render_value` actually emits 2-decimal forms (`$76.48B`) and ONLY renders total_supply/price/supply-momentum into scaffold sub-bullets — spot metrics (circulating_supply USD, market_cap, volume) and SEC financials live as RAW Evidence-Table numbers, which the WRITER re-renders at 3 SIGNIFICANT FIGURES (the brief's `$76.5B` example is the writer's 3-sf form, NOT `render_value`'s `$76.48B`). No single string passes a faithful report, so the gate accepts a value when ANY canonical rendering is present — the filler's exact `render_value`/`_signed_usd` form (reused from `fill` for parity) AND the 3-sf magnitude form (same T/B/M/K system), with sign-placement variants + Unicode-minus normalisation. The discriminating supply-momentum percent stays pinned to the filler-exact `{:+.2f}%`, so the required `-1.62% → -1.6%` alteration is still caught. Real report passes with ZERO false positives while staying a strict floor. Tests `tests/analysis_layer/qc/test_numbers.py` (9): faithful→[], altered %/magnitude/dropped value/dropped financial flagged, null + partially-null financials skipped, ★ GENERIC synthetic non-USDC bundle traces its own `ZYX` symbol (wrong-symbol supply flagged), live USDC report→[]. **Still open under TD-048:** ⑤.2 standardized confidence token + §C-cap enforcement; full narrative-number tracing → TD-049.
...
## TD-050 — v3 template refactor + GO-LIVE — ✅ COMPLETE for single-subject (USDC)

**Status:** GO-LIVE COMPLETE 2026-06-15 for single-subject. The live path is now v3 (`orchestrate.DEFAULT_TEMPLATE = crypto_research_v3.md`); the orchestrator's 2a filler places the machine facts table and the playbook-driven writer brief is live. Validated end-to-end on USDC (the only bound subject). Cross-subject 2-type validation is deferred to multi-subject (TD-046) — see "Remaining" #4. (Tracks work that previously existed only as untracked files with no TD — TD-023 grounding.)
...
**GO-LIVE steps (all done except the cross-subject gate, which is correctly deferred).**
1. ~~**Build the 2a filler**~~ — ✅ DONE (chunk 1, 2026-06-15). `analysis_layer/fillers/fill.py::fill_metrics_module(skeleton, facts)` + `build_metrics_table(facts)`: renders the bundle into the spot-metrics / supply-momentum / issuer-financials sub-tables (field-driven + graceful-missing like `charts.py`; subject-agnostic) and injects at `<!-- MODULE: metrics -->`. Reuses `render_value` / `_signed_usd` so numbers match the ⑤.1 gate + provenance forms. **★ Substring/prose trap (TD-023):** the anchor is matched LINE-ANCHORED (`(?m)^<!-- MODULE: metrics -->[ \t]*$`) with a verbatim replacement fn — so it neither catches the longer `metrics-analysis` anchor NOR the anchor's own prose mentions in the skeleton's ANCHOR CONVENTION blockquote (an earlier `str.replace` tripled the table off those two prose mentions). Only `metrics` is consumed; `metrics-analysis` (+ mechanism / comparison-matrix / valuation / risk-rows / thesis-breakers / charts) are left for the writer/renderer (unconsumed anchors still fail loud at render). Also landed the v3 skeleton polish chunk 1 needed: the separate `<!-- MODULE: metrics-analysis -->` anchor (writer's KEY SIGNAL + A–H dock, was colliding with `metrics`), the 0.1 "keep only the selected output-type row" instruction, and the 9.2 "module risk rows append to the one table" instruction. Tests: `tests/analysis_layer/test_metrics_filler.py` (10). **NOT done in this chunk:** repoint DEFAULT_TEMPLATE + the brief rewrite (below) — flipping the default before the brief is v3-aware would break output, so the filler is capability-only for now.
2. ~~**Repoint `orchestrate.DEFAULT_TEMPLATE`** → `crypto_research_v3.md`~~ — ✅ DONE (chunk 2–4, 2026-06-15). Done WITH step 3. **★ Reality vs assumption (TD-023):** the brief assumed "call `fill_metrics_module` after `build_facts_bundle`", but the orchestrator's scaffold producer was the **v2 `fill()`** ([AUTO]-slot + module-aware-section + Evidence-Table machinery), which does NOT apply to the v3 skeleton. So `build_report` now SELECTS the scaffold producer by template CONTENT (`_is_v3_skeleton` = the `<!-- MODULE: metrics -->` marker present): v3 → `fill_metrics_module(skeleton, facts)`; v2/v1.x → the legacy `fill()` (still supported via `--template`). The facts bundle is built ONCE in `build_report` and returned (7-tuple) so `_run` reuses it; `render_html(..., facts=facts)` already passed facts on the orchestrate surface (so charts + provenance render), and the `/research` step-5 render was fixed to pass `facts=` too (closes the audit's charts-gap). The v2-content e2e test was pinned to `--template v2` (it tests the still-live legacy machinery); a new `test_real_usdc_v3_default_scaffold` + `test_v3_live_path_scaffold_renders_and_traces` cover the v3 default.
3. ~~**Rewrite the live writer brief** to the playbook-driven flow~~ — ✅ DONE (chunk 2–4, 2026-06-15). `.claude/agents/crypto-report-writer.md` rewritten: read `subject_type` → classify §I → stack §II.0 → inject at v3 anchors (DELETE the `<!-- MODULE: … -->` comment, write prose in its place; only `charts` stays) → apply §III primitives (incl. one-home-per-insight) → apply §IV caps → strip coaching → return whole markdown. Points to the playbook as the authority. **Settles the verdict wobble:** the single-leg rule is pinned — with only the supply leg present (holder/usage = TD-043/044, not built), the KEY SIGNAL verdict stays PROVISIONAL and ticks NO row (never overclaim). Playbook block C promoted DRAFT → LIVE; `/research` command updated (scaffold description, the v3 writer prompt, and the facts-passing render).
4. **Cross-subject 2-type validation** — DEFERRED to multi-subject (TD-046). Only USDC is bound in `subject_ref._REGISTRY`, so a true second `subject_type` cannot be run end-to-end yet. This chunk validated v3 LIVE on **USDC** (1-type, automatic, e2e): `orchestrate USDC --html --bundle` auto-produces the scaffold + html + bundle (⑤.1 ✓, ④.3 ✓, 2 charts, provenance footer), and the `/research` writer pass produces the narrative `.report.md`/`.report.html` (⑤.1 ✓, ④.3 ✓, KEY SIGNAL once, peers `[MANUAL]`, verdict PROVISIONAL, no coaching leak). The 2-type gate pairs naturally with TD-046's second binding.

**Evidence / dry-runs (gitignored, regenerate on demand).** `meta/reports/usdc_v3_sample_*` (hand-driven), `usdc_v3_llm_*` (agent dry-run), and now the **LIVE** `usdc_<utc>.report.md`/`.html` produced by the wired path. A quality diff (live vs `usdc_v3_llm_*`) shows comparable-to-better: KEY SIGNAL once + Neutral + Rule-1 cap in both, the live report applies MORE Insight-Contract chaining and correctly holds the verdict PROVISIONAL (the dry-run had ticked CONTRACTION). All confirm §5.5 stays supply-leg-only (the holder/usage legs, TD-043/044, are the data-depth blockers independent of go-live).

**Known cosmetic (not blocking).** The CLI summary line still uses the v2-flavored heuristic (`filled slots=0 · derivations=none`) on a v3 run — the numbers live in the facts table, not `[AUTO]` slots, so the counters read zero. Harmless; tidy when convenient.
