...
- **Prefer reality over the literal prompt.** When a prompt asserts something about external reality — an API endpoint, a CIK, a response field, a coin's slug, a "canonical sibling" to copy — that assertion is a starting *hypothesis*, not ground truth. Verify before building. Silent build-on-stale-info ships wrong data; live-verify-then-surface-divergence is the discipline. This is the generalization of the narrow B.0 "prefer the sibling pattern over a prompt typo" lesson (TD-023).
  - **When it applies:** any API endpoint, response shape, or field name a prompt names; any "known fact" about a third-party system (IDs, slugs, CIKs, contract addresses, block heights); any "do X like the canonical sibling" reference (verify the sibling exists / is current); any "trust the top result" / "use `data[0]`" instruction (verify the list actually has collision protection).
  - **How to apply (priority order):** (1) **Probe live** before writing fetcher code — a 30-second `requests.get` is cheaper than a 1-hour build on a wrong assumption. (2) **Cross-reference canonical sources** in the repo (registry, spec files, prior siblings); if the prompt and a canonical source disagree, the canonical source is likelier right. (3) **Web-search current API behavior** when probing isn't safe (auth-gated or paid endpoints) — one corroborating result that the API moved is enough signal. (4) **Surface the divergence** in the agent response — show prompt-said vs reality-says, then propose the fix; never silently rewrite the envelope, the resolution logic, or the endpoint URL. (5) **Document the divergence** in the shipped artifacts (module docstrings, spec files, inline comments) so the next reader knows why the code disagrees with what the prompt would suggest.
  - **What this is NOT:** not license to ignore the prompt's *intent* — the intent (e.g. "resolve subject to a coin id, slug-first") is sacred; the *mechanism* (e.g. "via `/map?slug=`") is the falsifiable claim that may need correcting. Not license to silently "improve" the prompt — surfacing the divergence is half the value, because it updates the user's mental model of the API.
  ...
  - **Provenance:** B.1 fetcher work (B.0 + B.1.0 through B.1.7), 8 applications across 6 sub-tasks; 5 of the 8 caught bugs that would have shipped wrong data (CoinGecko symbol collision, Etherscan invalid-key field, SEC EDGAR Circle CIK, the CMC compound 3-error `/map` correction, L2Beat deprecated endpoints). Worked origin: `references/TODO.md` TD-023.
